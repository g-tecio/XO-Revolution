...
struct GameControls {
	...
	init(inThisScene: GameScene) {
        ...
        oform.name = "circle"
        oform.zPosition = 4
//        oform.position = CGPoint(x: gameScene.size.width/2, y: (gameScene.size.height*35/40))
        
        xform = SKSpriteNode.init(imageNamed: "cross")
        xform.name = "cross"
        xform.zPosition = 4
//        xform.position = CGPoint(x: gameScene.size.width/2, y: (gameScene.size.height*35/40))
